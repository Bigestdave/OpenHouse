...
import imgPropAdmiralty from '../assets/prop-admiralty.png'
import imgPropKitchen from '../assets/prop-kitchen.png'

const styles = [
  {
    id: 'lidar',
    title: 'Interactive LiDAR + Continuous Walkthrough',
    lines: ['High-density spatial point clouds', 'Fluid real-time room navigation'],
  ...
  {
    id: 'panoramic',
    title: 'High-Resolution 360° Spatial Tour',
...
export function CreateShowStyleScreen() {
  ...
  const handleContinue = () => {
    const styleTitle = styles.find((s) => s.id === selectedStyle)?.title || 'Interactive LiDAR + Continuous Walkthrough'
    const merged = {
      ...proposal,
    ...
    <WizardShell
      ...
      footerRight={
        <button
          ...
          className="flex items-center gap-2.5 rounded-lg bg-ink px-6 py-3 text-[15px] font-medium text-white transition-colors hover:bg-ink/90 shadow-subtle"
        >
          Continue to Publish
            ...
            return (
              <button
                ...
                className={`relative overflow-hidden rounded-xl border text-left transition-all shadow-subtle ${
                  isSel
                    ? 'border-accent bg-surface shadow-[0_0_0_2px_var(--color-accent)]'
                    : 'border-line bg-surface hover:border-line-strong'
                }`}
              >
                {s.recommended && (
                  <span className="absolute left-3 top-3 z-10 rounded-md bg-accent px-2.5 py-1 text-[12px] font-medium text-white shadow-subtle">
                    Recommended
                  </span>
                )}
                ...
                <div className="flex items-start justify-between p-5 bg-surface">
                  ...
                  <span
                    className={`mt-1 flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-full border transition-colors ${
                      isSel ? 'border-accent bg-accent text-white' : 'border-line'
                    }`}
                  >
                    {isSel && (
