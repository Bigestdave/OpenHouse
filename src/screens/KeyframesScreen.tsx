  ...
  return (
    ...
    >
      {selectedShot ? (
            ...
            <div>
              ...
              {isFailed && (
                <div className="mt-5 rounded-xl border border-line-soft bg-raised p-4">
                  <p className="flex items-center gap-2 pb-3 text-[14px] font-semibold text-ink">
                    <SparkleIcon size={15} />
                    OpenHouse recommends
                  </p>
              ...
              {isFailed ? (
                <>
                  <button
                    onClick={handleRetry}
                    disabled={acting}
                    className="w-full rounded-xl bg-accent px-5 py-3 text-[14.5px] font-semibold text-white transition-colors hover:bg-accent-strong disabled:opacity-50 shadow-subtle"
                  >
                    {acting ? '…' : 'Recapture this space angle'}
                  </button>
                  <button
                    onClick={handleApprove}
                    disabled={acting}
                    className="w-full rounded-xl border border-line bg-surface px-5 py-3 text-[14.5px] font-medium text-ink transition-colors hover:bg-raised disabled:opacity-50 shadow-subtle"
                  >
                    {acting ? '…' : 'Accept current perspective'}
                  </button>
                </>
              ) : (
                <button
                  onClick={handleApprove}
                  disabled={acting}
                  className="w-full rounded-xl bg-ink px-5 py-3 text-[14.5px] font-medium text-white transition-colors hover:bg-ink/90 disabled:opacity-50 shadow-subtle"
                >
                  {acting ? '…' : 'Approve spatial keyframe'}
                </button>
              )}
              <button
                onClick={handlePause}
        ...
        <div className="flex h-full items-center justify-center text-ink-2">Loading spatial keyframes…</div>
      )}
    </AppShell>
  )
}
